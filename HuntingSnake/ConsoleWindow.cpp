...
void LoadConsole()
{
	...
	// get font size use for the terminal's size calculation more precisely
	...
	SetConsoleSize(206, 50);
	SetConsolePosition((nScreenWidth - (getTermSize().x * fi.dwFontSize.X)) / 2 + 1, 0);
	DisableScrollbar();
	// After check, delete these 2 lines :D
	// expected: 200, 50
	GotoXY(0, 0);
	cout << getTermSize().x << ' ' << getTermSize().y << '\n';
...
BOOL SetConsoleSize(int cols, int rows) {
	// references: https://stackoverflow.com/questions/25912721/set-console-window-size-on-windows
	// author: anzz1 @ stackoverflow
	HWND hWnd;
	HANDLE hConOut;
	CONSOLE_FONT_INFO fi;
...
void mainMenu() {
	...
	//drawMenuSnake((int)(getTermSize().x - 82 + 1), (int)(getTermSize().y - 53),decorSnake_v2);
	//drawMenuSnake(0, 0,decorSnake);
	//drawHSnake(x_menu - (42 / 2), y_menu);

	//// draw menu
	//draw_rectangle(x_menu, y_menu, rec_height, rec_width, 5, BG_COLOR, "New Game", TXT_COLOR);
	//draw_rectangle(x_menu, y_menu + 5, rec_height, rec_width, 5, BG_COLOR, "Load Game", TXT_COLOR);
	//draw_rectangle(x_menu, y_menu + 10, rec_height, rec_width, 5, BG_COLOR, "Achievements", TXT_COLOR);
	//draw_rectangle(x_menu, y_menu + 15, rec_height, rec_width, 5, BG_COLOR, "Settings", TXT_COLOR);
	...
	while (true) {
		...
		if (_kbhit()) { // if any key is pressed
			...
			case 13: // if user pressed 'Enter'
				int selection = (y_pointer % y_menu) / (rec_height + 1);
				if (selection == 0) {
					StartGame();
					LoadMap();
					LoadGame();
					...
					draw_rectangle(2, 3, 5, 5, 2, 33, "123", 9);
				}
				else if (selection == 2) {
					draw_rectangle(2, 3, 5, 5, 9, 111, "bar", 9);
				}
				else if (selection == 3) {
					draw_rectangle(2, 3, 5, 5, 44, 3, "pon", 9);
				}
				break;
			}
		}
